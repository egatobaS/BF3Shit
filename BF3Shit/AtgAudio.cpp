...
VOID PlayPCM(IXAudio2* pXaudio2, const char* szFilename)
{
	...
	hasStopPlaying = false;

	while (SUCCEEDED(hr) && isRunning && shouldPlay)
	{
...
void PlayWav(const char* fileName)
{
	...
	while (!hasStopPlaying)
	{
		printf("Waiting on turning to true\n");
	...
	if (FAILED(hr = XAudio2Create(&pXAudio2, flags, 4)))
		printf("Error %#X calling XAudio2Create\n", hr);

	IXAudio2MasteringVoice* pMasteringVoice = NULL;

	if (FAILED(hr = pXAudio2->CreateMasteringVoice(&pMasteringVoice)))
		printf("Error %#X calling CreateMasteringVoice\n", hr);

	hasStopPlaying = true;
	shouldPlay = true;

	PlayPCM(pXAudio2, fileName);
...
void PlayThreadedWav(const char* Audio)
{
	CreateSystemThread(PlayWav, (void*)Audio);
}
